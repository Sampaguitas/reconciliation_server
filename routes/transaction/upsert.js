    ...
    if (!importId ) {
        ...
        .exec(function(errExport, exportitem) {
            if (!!errExport || !exportitem) {
                ...
                .findById(importId)
                .populate('importdoc')
                .exec(function(errImport, importitem) {
                    if (!!errImport || !importitem) {
                        res.status(400).json({message: 'Could not retreive impotItem information.'});
                    } else {
                        let summary = importitem.importdoc.summary.find(element => _.isEqual(element.hsCode, importitem.hsCode) && _.isEqual(element.country, importitem.country));
                        if (_.isUndefined(summary)) {
                            res.status(400).json({message: 'Could not retreive import summary information.'});
                        } else if ( ((summary.totalPrice / summary.pcs) / (exportitem.exportdoc.exRate || 1) ) > exportitem.unitPrice ){
